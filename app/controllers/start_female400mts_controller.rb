class StartFemale400mtsController < ApplicationController
  ...
  def index
     ...
     format.html
      format.json { render json: StartFemale400mtsDatatable.new(view_context) }
      format.xlsx
    end
end

def import
StartFemale400mt.import(params[:file])
redirect_to start_female400mts_url, notice: "Importado correctamente"
end
  ...
  def create
    ...
    respond_to do |format|
      if @start_female400mt.save
        format.html { redirect_to action:index, notice: 'Start female400mt was successfully created.' }
