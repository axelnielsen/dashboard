class StartFemale100mthudrlesController < ApplicationController
  ...
  def index
     ...
     format.html
      format.json { render json: StartFemale100mthudrlesDatatable.new(view_context) }
      format.xlsx
    end
end

def import
StartFemale100mthudrle.import(params[:file])
redirect_to start_female100mthudrles_url, notice: "Importado correctamente"
end
  ...
  def create
    ...
    respond_to do |format|
      if @start_female100mthudrle.save
        format.html { redirect_to action:index, notice: 'Start female100mthudrle was successfully created.' }
